...
export default {
    ...
    paths: {
         ...
         *
         * 想指向别处就改成**绝对路径** —— 移动硬盘、Photos 导出目录、
         * NAS / 网盘的挂载点，或者容器里的 /data/photos。
         * 这条是实测验证过的：扫描、上传落盘、缩略图、原图与视频 Range 全部正常。
         *
         * 放挂载点时的四条经验（改完跑 `npm run doctor`，它会逐条实测）：
         *   · 缩略图缓存（thumbs.cacheDir）**留在本地盘**，别跟着放到网盘 ——
         *     九宫格每个格子都要随机读原图，走网络会慢到不可用。
         *   · 🔴 mediaRoot 与 cacheDir 要落在**同一个挂载点**下，否则上传入库
         *     会丢掉原子性：Linux 的 rename(2) 按挂载点判断，跨挂载点返回 EXDEV，
         *     入库退化成"复制 + 删除"，目标目录逐步出现 —— 一批上传途中读者
         *     可能看到只有一半文件的动态。容器里别把两者挂成两个 -v，
         *     挂一个 /data 让 photos/ 与 cache/ 做它的子目录（见 docker-compose.yml）。
         *   · 网络盘上 upload.naming 建议改成 'time'：'order' 起名要"读出已用序号
         *     再 +1"，靠的是 mkdir 目录锁，而网络盘不保证 mkdir 的原子性。
         *     'time' 直接生成不重复的名字，**连锁都不依赖**。
         *   · server.watch 建议关掉：网络盘不支持文件系统事件，开着也不会自动刷新。
         */
        mediaRoot: 'photos'
    },
    ...
    server: {
        port: 4310,
        /**
         * 监听地址。想让**局域网里其他设备**访问（手机、其他电脑共用一个库），
         * 就改成 '0.0.0.0'，再用本机 IP 访问。
         */
        host: '127.0.0.1',
        /**
         * 监听照片目录变化，自动刷新页面。
         * 照片库在网络盘 / 网盘挂载点上时建议设成 **false** —— 那类文件系统
         * 不支持文件系统事件，开着也建不起监听（启动时会提示一次，之后手动刷新即可）。
         */
        watch: true
    }
};
